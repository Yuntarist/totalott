...
export default {
  ...
  mounted: function () {
    ...
    const PWSuccessMessage = document.querySelector('.password-success-message')
    ...
    // [이메일] 매치가 동일하면 true를 리턴하는 함수
    // 숫자(0~9), 알파벳(a~z,A~Z)으로 시작하고 중간에 -_.사용가능하고
    // @링 .은 반드시 존재해야하고 i = 대소문자 구분안한다.
    function isEmail(emailAddress) {
      let patterns =
        /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/i

      if (emailAddress.match(patterns)) {
        return true
      } else {
        return false
      }

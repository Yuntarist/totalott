...
(async () => {
  ...
  for (var v of urlKeys) {
    ...
    console.log(v + " (title)", steam_titles); // 게임이름
    console.log(v + " (price)", steam_price); // 할인되기 전 정가
    console.log(v + " (dispercent)", steam_discount_percent); // 할인율
    console.log(v + " (disprice)", steam_discount_price); // 할인가
  }
  await browser.close();
})();
// 상위 20개만 다른사이트로 해보기
